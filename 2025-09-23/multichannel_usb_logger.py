...
class CO2LoggerApp:
    def __init__(self, root):
        ...
        # Add CO2 sensor checkboxes with color swatch (only for responsive ports)
    ...
    def _update_stats(self):
        ...
        for i in range(6):
            ...
            for lbl, val in zip(self.stat_labels['co2'][i], (fmt(c, 0), fmt(mn, 0), fmt(mx, 0), fmt(a, 1))):
                lbl.configure(text=val)
        for j in range(2):
            c, mn, mx, a = stats(self.temp_series[j])
            for lbl, val in zip(self.stat_labels['temp'][j], (fmt(c, 1), fmt(mn, 1), fmt(mx, 1), fmt(a, 1))):
                lbl.configure(text=val)
        for j in range(2):
            c, mn, mx, a = stats(self.hum_series[j])
            for lbl, val in zip(self.stat_labels['hum'][j], (fmt(c, 1), fmt(mn, 1), fmt(mx, 1), fmt(a, 1))):
                lbl.configure(text=val)
