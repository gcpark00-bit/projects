...
class CalApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # Bottom bar with EXIT button (bottom-right)
    ...
    def _discover_co2_ports(self):
        ...
        for p in ports:
            ...
            if 'cu.usbserial' in dev.lower() or 'cu.bluetooth' in dev.lower() or 'bluetooth' in blob or 'debug' in blob:
                continue
            # 1) Prefer VID routing: Silicon Labs CP210x (VID 0x10C4)
            if isinstance(vid, int) and vid == 0x10C4:
                co2.append(dev); continue
            # 2) macOS path / descriptor routing
            if ('cu.slab' in blob) or ('cp210' in blob) or ('silicon labs' in blob) or ('slab' in blob):
                co2.append(dev)
                continue
            # Skip WCH/CH340 (TH sensors per rule)
